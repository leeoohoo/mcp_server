...
export class FsOps {
  ...
  async readFileRaw(relPath: string) {
  ...
  async readFileRange(relPath: string, start: number, end: number, withLineNumbers: boolean) {
  ...
  async listDir(relPath: string, maxEntries = 200): Promise<FileEntry[]> {
    const target = this.resolvePath(relPath || '.');
  ...
  async searchText(pattern: string, relPath = '.', maxResults?: number) {
  ...
  async writeFile(relPath: string, content: string) {
    if (!this.allowWrites) throw new Error('Writes are disabled.');
    const target = this.resolvePath(relPath);
  ...
  async appendFile(relPath: string, content: string) {
    if (!this.allowWrites) throw new Error('Writes are disabled.');
    const target = this.resolvePath(relPath);
    const buffer = Buffer.from(content, 'utf8');
    if (buffer.length > this.maxWriteBytes) {
  ...
  async deletePath(relPath: string) {
    if (!this.allowWrites) throw new Error('Writes are disabled.');
    const target = this.resolvePath(relPath);
