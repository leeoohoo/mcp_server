...
const serverName = normalizeName(
  String(args.name || process.env.MCP_SERVER_NAME || 'shell_mcp'),
  'shell_mcp'
);

const workspaceRoot = normalizeId(args.root || args.workspace || process.env.MCP_WORKSPACE_ROOT) || process.cwd();
const defaultTimeoutMs = clampNumber(
  args['timeout-ms'] || args.timeout || process.env.MCP_SHELL_TIMEOUT_MS,
  1000,
  60 * 60 * 1000,
  5 * 60_000
);
const maxOutputBytes = clampNumber(
  args['max-output-bytes'] || process.env.MCP_SHELL_MAX_OUTPUT_BYTES,
  1024,
  50 * 1024 * 1024,
  5 * 1024 * 1024
);
...
async function main() {
  ...
  console.error(
    `[${serverName}] shell MCP server ready (root=${workspaceRoot}, timeout=${defaultTimeoutMs}ms, maxOutput=${maxOutputBytes} bytes).`
  );
}
