    ...
    const server = http.createServer(async (req, res) => {
        try {
            ...
            if (req.method === 'POST' && url === '/api/mcp_servers/save') {
                ...
                if (!name)
                    return sendJson(res, { ok: false, error: 'missing name' }, 400);
                if (transport === 'stdio') {
                    if (!command)
                        return sendJson(res, { ok: false, error: 'missing command' }, 400);
                }
                else {
                    if (!endpointUrl)
                        return sendJson(res, { ok: false, error: 'missing endpoint url' }, 400);
                }
            ...
            if (req.method === 'POST' && url === '/api/mcp_servers/delete') {
                ...
                const id = typeof body?.id === 'string' ? body.id.trim() : '';
                if (!id)
                    return sendJson(res, { ok: false, error: 'missing id' }, 400);
            ...
            if (req.method === 'POST' && url === '/api/plugins/install') {
                ...
                if (!source)
                    return sendJson(res, { ok: false, error: 'missing source' }, 400);
                const summary = buildMarketplaceSummary(options);
                const plugin = summary.plugins.find((entry) => entry.source === source);
                if (!plugin)
                    return sendJson(res, { ok: false, error: 'plugin not found' }, 404);
...
function parseCommandParts(commandInput, argsInput) {
...
function buildMarketplaceSummary(options) {
    ...
    const summaryPlugins = plugins.map((plugin) => {
...
function loadMarketplaceJson(options) {
...
function resolvePluginRoot(source, root) {
    if (!source)
        return root;
...
function mapEntries(pluginRoot, entries) {
    const list = [];
    for (const entry of entries || []) {
...
function countAvailability(items) {
    const total = items.length;
    const available = items.filter((item) => item.exists).length;
    return { total, available };
}
function aggregateCounts(plugins, key) {
...
async function installPlugin(options, plugin, summary) {
    const marketplaceDir = options.marketplacePath ? path.dirname(options.marketplacePath) : process.cwd();
    ...
    if (!destRoot)
        return { ok: false, error: 'plugins_root not set' };
...
function resolveGitCacheRoot(options, marketplaceDir) {
...
async function ensureGitRepo(repoUrl, cacheRoot) {
...
async function updateGitRepo(repoPath) {
    try {
...
async function resolveOriginHead(repoPath) {
    try {
