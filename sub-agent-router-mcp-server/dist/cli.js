...
const registryPath = normalizeId(args.registry) ||
    normalizeId(process.env.MODEL_CLI_SUBAGENT_REGISTRY) ||
    path.join(stateDir, 'subagents.json');
ensureDir(path.dirname(registryPath));
const marketplacePath = normalizeId(args.marketplace) ||
...
const pluginsRoot = normalizeId(args['plugins-root']) ||
...
const timeoutMs = parseNumber(args['timeout-ms']) ||
    ...
    120000;
const maxOutputBytes = parseNumber(args['max-output-bytes']) ||
    parseNumber(process.env.SUBAGENT_MAX_OUTPUT_BYTES) ||
    1024 * 1024;
const llmTimeoutMs = parseNumber(args['llm-timeout-ms']) ||
    parseNumber(process.env.SUBAGENT_LLM_TIMEOUT_MS) ||
    180000;
const llmMaxOutputBytes = parseNumber(args['llm-max-output-bytes']) ||
...
async function main() {
    const adminPort = parseNumber(args['admin-port']) ||
    ...
    console.error(`[${serverName}] Sub-agent router ready (db=${dbPath}, registry=${registry.getFilePath()}, marketplace=${marketplacePath}).`);
}
main().catch((err) => {
