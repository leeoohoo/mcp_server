...
const SUBAGENT_GUARDRAIL = 'Tooling guard: sub-agents cannot call mcp_subagent_router_* or other sub-agent routing tools. Complete the task directly with available project/shell/task tools.';
export function createSubAgentRouterServer(options) {
    const { serverName, catalog, jobStore, configStore, defaultSessionId, defaultRunId, timeoutMs, maxOutputBytes, ai, } = options;
    const inflight = new Map();
    const cancelled = new Set();
    const server = new McpServer({ name: serverName, version: '0.2.0' });
    server.registerTool('get_sub_agent', {
    ...
    server.registerTool('suggest_sub_agent', {
    ...
    server.registerTool('run_sub_agent', {
        title: 'Run sub-agent',
    ...
    server.registerTool('start_sub_agent_async', {
    ...
    server.registerTool('get_sub_agent_status', {
    ...
    server.registerTool('cancel_sub_agent_job', {
...
function resolveAgentAndCommand(input, catalog) {
    const requestedId = normalizeId(input.agent_id);
    if (requestedId) {
        const agent = catalog.getAgent(requestedId);
        if (!agent)
            throw new Error(`Sub-agent ${requestedId} not found.`);
...
async function suggestWithAi(options) {
    ...
    if (!parsed)
        return null;
    const agentId = normalizeId(parsed.agent_id);
    if (!agentId)
        return null;
...
function filterAgents(agents, options) {
    ...
    if (options.commandId) {
        ...
        filtered = filtered.filter((agent) => (agent.commands || []).some((cmd) => normalizeId(cmd.id).toLowerCase() === target || normalizeId(cmd.name).toLowerCase() === target));
    }
    if (options.query) {
        const q = options.query.trim().toLowerCase();
        filtered = filtered.filter((agent) => `${agent.id} ${agent.name} ${agent.description || ''}`.toLowerCase().includes(q));
    }
    return filtered;
}
function selectSkills(agent, inputSkills, catalog) {
...
function resolveSkillIds(skillIds, agent) {
    if (!agent)
        return skillIds;
...
function buildSystemPrompt(agent, skills, command, catalog, allowPrefixes) {
    const sections = [];
...
function buildPrompt(systemPrompt, task) {
    return `SYSTEM:\n${systemPrompt}\n\nUSER:\n${task}\n`;
}
function buildAiMeta(agent, command, ctx) {
...
function ensureAiConfigured(ai) {
...
function hasAiConfig(ai) {
    return !!ai.http && !!ai.http.apiKey && !!ai.http.model;
}
function resolveAiConfig(ai, configStore, runtime) {
...
function resolveRuntimeConfig(configStore, defaults) {
...
function normalizeRuntimeValue(value, fallback) {
    if (value === undefined || value === null || Number.isNaN(value))
        return fallback;
    return value;
}
function resolveAllowPrefixes(input, configStore, mcpServers) {
...
function withChatos(serverName, tool, payload, status = 'ok') {
    return {
        ...payload,
        chatos: {
            status,
