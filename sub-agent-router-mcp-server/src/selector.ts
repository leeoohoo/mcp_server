import { AgentSpec, CommandSpec } from './types.js';
...
export function pickAgent(agents: AgentSpec[], options: PickOptions): PickResult | null {
  ...
  for (const agent of list) {
    ...
    let score = 0;
    if (desiredCategory && agentCategory && desiredCategory === agentCategory) score += 4;
    score += skillMatches.length * 3;
    score += queryMatches.length * 2;
    score += taskMatches.length;
    if (commandId && matchedCommand) score += 5;

    const usedSkills = desiredSkills.length > 0 ? desiredSkills : agentSkills;
    const reasonParts: string[] = [];
    if (desiredCategory && agentCategory === desiredCategory) reasonParts.push(`category:${desiredCategory}`);
    if (skillMatches.length > 0) reasonParts.push(`skills:${skillMatches.join(',')}`);
    if (queryMatches.length > 0) reasonParts.push(`query:${queryMatches.join(',')}`);
    if (taskMatches.length > 0) reasonParts.push(`task:${taskMatches.join(',')}`);
    if (commandId && matchedCommand) reasonParts.push(`command:${commandId}`);

    const reason = reasonParts.length > 0 ? reasonParts.join(' | ') : 'Best available match';
    const current: PickResult = {
      agent,
...
function resolveCommand(commands: CommandSpec[], commandId?: string): CommandSpec | null {
  const id = normalizeToken(commandId);
  if (!id) return null;
  return (
    commands.find((cmd) => normalizeToken(cmd.id) === id) ||
    commands.find((cmd) => normalizeToken(cmd.name) === id) ||
    null
  );
}

export function pickDefaultCommand(commands: CommandSpec[], preferredId?: string): CommandSpec | null {
  const preferred = resolveCommand(commands, preferredId);
  if (preferred) return preferred;
  return commands.length > 0 ? commands[0] : null;
}

function flattenCommandTokens(commands: CommandSpec[]): string[] {
  const tokens: string[] = [];
